...
def draw_row(ctx, fn, count, cx, cy):
    ...
    if count <= 3:
        ...
        scale = 0.23  # untuk 7-8 buah

    if count == 4:  # kanan: 4 buah
        fn(ctx,cx-spacing_x/2, cy-spacing_y/4, scale)
        fn(ctx,cx+spacing_x/2, cy-spacing_y/4, scale)
        fn(ctx,cx-spacing_x/2, cy+spacing_y/4, scale)
        fn(ctx,cx+spacing_x/2, cy+spacing_y/4, scale)

    elif count == 8:  # kiri: 3–3–2 layout
        # baris 1
        ...
        fn(ctx,cx+spacing_x,   cy, scale)
        # baris 3
        fn(ctx,cx-spacing_x/2, cy+spacing_y/2, scale)
        fn(ctx,cx+spacing_x/2, cy+spacing_y/2, scale)
...
def make():
    ...
    draw_row(ctx, draw_banana, 8, 450, cy)
    draw_symbol(ctx, "-", 800, cy)
    draw_row(ctx, draw_apple, 4, 1150, cy)
    draw_symbol(ctx, "=", 1400, cy)

    surface.write_to_png("assets/soalPengurangan8.png")
<<<<<<< HEAD
    print("✔ Saved soalPengurangan8.png")
