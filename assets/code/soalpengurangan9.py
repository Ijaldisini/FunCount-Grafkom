...
def draw_row_left(ctx, fn, count, cx, cy):
    ...
    for r in range(rows):
        ...
        for c in (-1, 0, 1):  # x offset kiri, tengah, kanan
            if idx >= count:
                break
            x = cx + c * spacing_x
            fn(ctx, x, y, scale)
...
def make():
    surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 1600, 700)
    ...
    draw_row_left(ctx, draw_apple, 9, 380, cy)
    draw_symbol(ctx, "-", 750, cy)
    draw_row_right(ctx, draw_apple, 5, 1080, cy)
    draw_symbol(ctx, "=", 1350, cy)

    surface.write_to_png("assets/soalPengurangan9.png")
    print("✔ Saved soalPengurangan9.png")
